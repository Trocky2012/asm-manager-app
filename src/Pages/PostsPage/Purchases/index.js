...
class Purchases extends Component{
  ...
  async fcnSpecificPost(postId, visibility){
    ...
    if(visibility){
      try{
          const getPost = 'purchase/'+postId.toString();
...
// -------------------------------------------------------------------
  ...
  render(){
    ...
    const { id, price, update_time, insert_time, feedstock } = this.props.data;
            ...
            <View style={styles.card}>
              ...
              <View style={{borderBottomWidth:0.5,borderColor:'#CD853F', marginBottom:10, flexDirection:'row', paddingRight:10,paddingLeft:2, 
                ...
                <Text style={styles.titulo}>{insert_time == null? "-" : format(new Date(insert_time), "MMMM do, yyyy - H:mma")}</Text>
                    <Text style={{backgroundColor:'#5da9FF', color:'#FFF', borderRadius:20, fontSize:15, padding:5}}>   $ {parseFloat(price).toFixed(2)}   </Text>
                  
              </View>
            

              <FlatList
                data={this.state.specificPost.feedstock}
