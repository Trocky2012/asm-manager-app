...
class SimpleFeeds extends Component{
  ...
  async addFeedsToNewPurchase(id, transient_quantity, actionId, action){
    if(transient_quantity.length > 0){
      ...
      try{
        ...
        {
          "id": id.toString(),
	        "transient_quantity": ((parseFloat(transient_quantity).toFixed(2)).toString())
        });
        
        // const apiResponse = 
        await api.put(action+'/'+actionId+'/add-feeds',newData);
  ...
  render(){
    ...
    const { id, name, price, quantity, unity} = this.props.data;
        ...
        <ScrollView>
         ...
         {/* MAIN SCREEN - LIST OS PRODUCT*/}
          ...
          <View style={{borderBottomWidth:0.5,borderColor:'#BBB', marginBottom:10, flexDirection:'row', paddingRight:5,
            ...
            {action=='purchase' ?
              <Text style={{borderWidth:1,borderColor:'#6495ED', color:'#6495ED', borderRadius:20, fontSize:14, padding:6, paddingRight:10, paddingLeft:10}}>{(parseFloat(quantity)+parseFloat(this.state.addQuantity)).toFixed(2)} {unity}</Text>
              :
          ...
          <View>
              <View style={{marginBottom:0, marginTop:5, flexDirection:'row', justifyContent:'flex-start', alignItems:'center', 
                  ...
                  <Text style={{fontSize:13, padding:10, paddingLeft:7}}>- Cost: </Text>
                  ...
                  {/* <Text style={styles.texto}>Quantity: {quantity} {unity}</Text> */}
              </View>
                ...
                {this.state.showSetQtdAndSend ? 
                  <View style={{flexDirection:'row', marginTop:10, paddingTop:10, borderTopWidth:0.5, borderTopColor:'#2F6F4F', justifyContent:'space-between', 
                      ...
                      <TextInput 
                        ...
                        onChangeText={this.setAddQuantity}
                        onSubmitEditing={() => { this.addFeedsToNewPurchase(id,(this.state.addQuantity),actionId, action) }}
                      />
                      <Text style={{marginLeft:-25, fontSize:10,color:'#CD964F'}}>- {unity}</Text>
